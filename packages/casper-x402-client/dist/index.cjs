...
__export(index_exports, {
  CASPER_MAINNET_DOMAIN: () => CASPER_MAINNET_DOMAIN,
  CASPER_TESTNET_DOMAIN: () => CASPER_TESTNET_DOMAIN,
  buildTransferAuthorization: () => buildTransferAuthorization,
  computeAccountHash: () => computeAccountHash,
  createCasperPaymentHeader: () => createCasperPaymentHeader,
  createCasperSigner: () => createCasperSigner,
  fromHex: () => fromHex,
  selectCasperPaymentRequirements: () => selectCasperPaymentRequirements,
...
function computeAccountHash(algoPrefix, rawPublicKeyHex) {
  ...
  const hash = (0, import_blake2b.blake2b)(input, { dkLen: 32 });
...
function createCasperSigner(privateKeyHex, algorithm = "ed25519") {
  ...
  if (algorithm === "ed25519") {
    const pubKey = import_ed25519.ed25519.getPublicKey(privKeyBytes);
    rawPublicKeyHex = toHex(pubKey);
    algoPrefix = "01";
  } else {
    const pubKey = import_secp256k1.secp256k1.getPublicKey(privKeyBytes, true);
    rawPublicKeyHex = toHex(pubKey);
    algoPrefix = "02";
  }
  ...
  return {
    ...
    async sign(digest) {
      if (algorithm === "ed25519") {
        return import_ed25519.ed25519.sign(digest, privKeyBytes);
      } else {
        const sig = import_secp256k1.secp256k1.sign(digest, privKeyBytes);
...
// src/client.ts
var import_casper_eip_712 = require("@casper-ecosystem/casper-eip-712");
...
async function signTransferAuthorization(signer, domain, message) {
  const digest = (0, import_casper_eip_712.hashTypedData)(
    domain,
    import_casper_eip_712.TransferAuthorizationTypes,
