...
export const ROUTE_REGISTRY = [
  ...
  },
  {
    ...
    method: 'GET',
    operationId: 'syncTileChainGet',
    summary: '(Internal) Sync on-chain tile state to DB across chains',
    description: 'Scans Base mint events and can verify known Casper tile IDs with wallet + tileIds query params.',
    tags: ['tiles', 'admin'],
    params: [
      { name: 'chain', in: 'query', schema: { type: 'string', enum: ['all', 'base', 'casper'], default: 'all' } },
      { name: 'wallet', in: 'query', schema: { type: 'string' } },
      { name: 'tileIds', in: 'query', schema: { type: 'string' }, description: 'Comma-separated tile IDs for Casper verification' },
    ],
  ...
  },
  {
    ...
    description: 'Called by cron or webhooks to reconcile on-chain ownership with tiles.bot DB. Defaults to Base for backward compatibility; pass chain to target Casper or Base.',
    tags: ['tiles', 'admin'],
    responses: { '200': { description: 'Sync result' } },
  },
